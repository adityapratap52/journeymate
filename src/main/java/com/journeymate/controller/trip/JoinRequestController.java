...
public class JoinRequestController {
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved join request"),
    ...
    @Operation(
        summary = "Get join requests by trip post ID",
    ...
    @Operation(
        summary = "Get join requests by user ID",
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "Join request updated successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid input"),
            @ApiResponse(responseCode = "404", description = "Join request not found"),
            @ApiResponse(responseCode = "403", description = "Access forbidden")
        }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or @joinRequestService.getJoinRequestById(#id).userId == authentication.principal.username")
    public ResponseEntity<JoinRequestDTO> updateJoinRequest(@PathVariable Long id, @RequestBody JoinRequestDTO joinRequestDTO) {
        return ResponseEntity.ok(joinRequestService.updateJoinRequest(id, joinRequestDTO));
    }
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "Join request deleted successfully"),
            @ApiResponse(responseCode = "404", description = "Join request not found"),
            @ApiResponse(responseCode = "403", description = "Access forbidden")
        }
    )
